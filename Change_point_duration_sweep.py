"""Duration sweep: detection rate and EDD for all six methods across a
range of change durations, with magnitude held fixed at the KL-matched
baseline used in Section 4.2."""
...
OUTPUT_DIR = BASE_DIR / "outputs" / "duration_sweep"
...
T = 500
CHANGEPOINT = 250
BASELINE_DELTA = 2.0          # magnitude held fixed -- duration is the swept variable here
N_REPS = 500
BASE_SEED = 0

DURATION_SWEEP = [1, 2, 3, 5, 7, 10, 15, 20, 25, 35, 50, 75, 100, 150, 250]
...
# Locked calibrated parameters (KL-matched baseline, duration=25)
PARAMS = {
...
BASELINE_VARIANCE_MULTIPLIER = kl_matched_variance_multiplier(BASELINE_DELTA)
...
def detection_rate_error(result):
    ...
    n = result["n_reps"]
    return np.sqrt(p * (1 - p) / n)  # fraction, 0-1 scale
...
def run_sweep():
    """Run every method against every duration in DURATION_SWEEP, with
    shift magnitude held fixed at the KL-matched baseline, for both
    scenarios."""

    print(f"Magnitude held fixed: spike_size_std={BASELINE_DELTA}, "
          f"variance_multiplier={BASELINE_VARIANCE_MULTIPLIER:.4f}")
    print(f"Sweeping duration: {DURATION_SWEEP}\n")
    ...
    for name, detector in METHOD_FUNCS.items():
        ...
        for duration in DURATION_SWEEP:
            mean_kwargs = dict(T=T, changepoint=CHANGEPOINT, spike_size_std=BASELINE_DELTA,
                                duration=duration, autocorrelation=0.0)
            var_kwargs = dict(T=T, changepoint=CHANGEPOINT, variance_multiplier=BASELINE_VARIANCE_MULTIPLIER,
                               duration=duration, autocorrelation=0.0)
            ...
            print(f"{name}: duration={duration} -> "
                  f"mean_shift det={r_mean['detection_rate']*100:.1f}% "
...
def plot_stacked(results, y_field, y_label, title_mean, title_var, filename, error_mode=ERROR_BAR_MODE):
    ...
    of either detection rate or EDD against duration, one line per method
    with shaded error bands."""

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(9, 8.5), sharey=(y_field == "detection_rate"))

    EDD_FLOOR = 0.1   # log scale can't show 0 or negative -- clip the shaded band's lower edge here
    ...
    for ax, scenario, title in [(ax1, "mean_shift", title_mean), (ax2, "variance_change", title_var)]:
        for name in METHOD_STYLE:
            ...
            x = [r["x"] for r in rows]   # duration -- identical for both panels
        ...
        ax.axvline(25, color="#8a8a8a", linestyle=":", linewidth=1.3, zorder=0)
...
if __name__ == "__main__":
    ...
    plot_stacked(results, "detection_rate", "Detection rate (%)",
                 ...
                 OUTPUT_DIR / "duration_sweep_detection_rate_stacked.png")

    plot_stacked(results, "EDD", "Expected detection delay",
                 "Mean-shift Scenario", "Variance-change Scenario",
                 OUTPUT_DIR / "duration_sweep_edd_stacked.png")
